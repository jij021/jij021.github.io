...
let sketch = function(p) {
  ...
  function HSLrandom() {
    ...
    if((between(hue1, 70, 135) && between(hue2, 190, 290)) || (between(hue2, 70, 135) && between(hue1, 190, 290))){
      do {hue1 = getRandomInt(0, 361);} 
  ...
  p.setup = function() {
    p.colorMode(p.HSB);
    const canvas = p.createCanvas(sizeW, sizeH);
    canvas.parent("animation");
    canvas.style("display", "block");
    cols = p.floor(sizeW/scl);
    rows = p.floor(sizeH/scl);

    if(piInt == 1){direction = p.TWO_PI;} 
    if(piInt == 2){direction = p.PI * p.PI;} 
    if(piInt == 3){direction = p.PI * 4;} 
    if(piInt == 4){direction = p.QUARTER_PI * getRandomInt(6,17);} 
    ...
    p.background(hue1, sat1, light1);
  };

  p.draw = function() {
    // sun
    p.noStroke();
    p.fill(hue2, sat2, light2);
    if(qInt == 1){p.circle(p.width/4, p.height/8, r);} 
    if(qInt == 2){p.circle((p.width/4)*3, p.height/8, r);} 
    if(qInt == 3){p.circle(p.width/4, (p.height/8)*3, r);} 
    if(qInt == 4){p.circle((p.width/4)*3, (p.height/8)*3, r);} 
    if(qInt == 5){p.circle(p.width/2, p.height/4, r);}
    ...
    for (var y = 0; y < rows; y++){
      ...
      for (var x = 0; x < cols; x++){
        ...
        var index = x + y * cols;
        var angle = p.noise(xoff, yoff, zoff) * direction;
